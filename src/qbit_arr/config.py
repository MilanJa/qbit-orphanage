...
class PathsConfig(BaseSettings):
    ...
    torrent_movies: Path = Field(
        default=Path("/data/media/torrents/movies"), description="Path to torrent movies directory"
    )
    torrent_tv: Path = Field(
        default=Path("/data/media/torrents/tv"), description="Path to torrent TV shows directory"
    )
    library_movies: Path = Field(
        default=Path("/data/media/libraries/movies"), description="Path to movie library directory"
    )
    library_tv: Path = Field(
        default=Path("/data/media/libraries/tv"), description="Path to TV library directory"
    )
    ...
    remote_path_base: str = Field(
        default="/media", description="Base path as reported by qBittorrent/Radarr/Sonarr"
    )
    local_path_base: str = Field(
        default="/data/media", description="Actual filesystem path where files are located"
    )
