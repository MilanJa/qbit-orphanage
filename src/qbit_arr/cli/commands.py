...
@click.group()
@click.option(
    "--config", type=click.Path(exists=True, path_type=Path), help="Path to configuration file"
)
@click.pass_context
...
def orphans(ctx: click.Context, output_json: bool) -> None:
    ...
    if output_json:
        ...
        console.print(
            json.dumps([o.model_dump(mode="json") for o in orphaned], indent=2, default=str)
        )
    else:
...
def hardlinks(ctx: click.Context, output_json: bool) -> None:
    ...
    if output_json:
        ...
        console.print(
            json.dumps([g.model_dump(mode="json") for g in groups], indent=2, default=str)
        )
    else:
